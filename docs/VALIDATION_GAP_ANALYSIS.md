...
| # | UC | Story | Observation | Code Location | Real Bug? | Effort |
|---|-----|-------|-------------|---------------|-----------|--------|
| M1 | UC-02 | UIB-35 AC1-7 | **Page numbers shown but proper citations not displayed.** Validator: "Page numbers are displayed for all answers, but proper citations are not shown." Citation format missing: high-level location, version/date, deep links. | `agent.py:132` (REACT_TEMPLATE) instructs `Source: [filename], Page: [number]` only. `models.py:29-32` (`SourceDoc`) has only `source`, `page`, `snippet` — no `location`, `version`, `url` fields. `tools.py:36-37` (`_format_docs`) outputs only name + page. | **YES — real gap.** Citation schema lacks location hierarchy, version/date, and deep links required by UIB-35 AC4/AC7. Not a regression — was never implemented. | Medium |
...
| M4 | UC-06 (Conversational Context) | UIB-93 AC4-7 | **Context points 4,5,6,7 unverified.** Validator marks all as "needs verification." | Point 4 (configurable context window): `config.py:84` — `max_history_turns: int = 10`, env-configurable only, no admin UI. `agent.py:177-182` — `ConversationBufferWindowMemory(k=settings.max_history_turns)`. Point 5 (context encryption): ❌ not encrypted — plaintext Python dicts in memory. Point 6 (context timeout): ❌ no TTL — `_sessions` dict at `agent.py:76` persists indefinitely until explicit `clear_session()`. Point 7 (multi-device): ⚠️ session reuse by ID at `agent.py:204-216` — same session_id from different devices shares memory. | **Real gaps (4 of 4).** Context window is env-configurable (partial pass on point 4). Points 5, 6, 7 are not implemented. | Large |
| M5 | UC-02 | UIB-18 AC4 | **Session expiry functionality needs verification.** | JWT has 8-hour expiry (`auth.py:74`, `config.py:40`). But agent sessions (`_sessions` dict at `agent.py:76`) have **no TTL**. A session created at hour 0 persists in memory even after JWT expires at hour 8. New requests with a fresh JWT + same session_id reattach to the stale executor. | **YES — real gap.** JWT expires but agent memory does not. Memory leak risk on long-running App Runner instances. | Small |
